...
echo "[task.sh] [1/9] Downloading ingestion config from $INGESTION_CONFIG_URL."
wget $INGESTION_CONFIG_URL -O config.json

echo "[task.sh] [2/9] Updating date strings in config if requested."
if [ $UPDATE_DATES_IN_CONFIG ] then
    python3 /actor/update_date_string_in_config.py config.json
fi

echo "[task.sh] [3/9] Downloading list of files to ingest from $INGESTION_FILES_LIST_URL."
wget $INGESTION_FILES_LIST_URL -O file_list.json

echo "[task.sh] [4/9] Updating date strings in file list."
python3 /actor/update_date_string_in_config.py file_list.json

echo "[task.sh] [5/9] Pulling files to ingest."
python3 /actor/wget_from_json_list.py file_list.json

echo "[task.sh] [6/9] Getting and setting authentication token."
python3 /actor/get_auth_token.py IW_TOKEN

echo "[task.sh] [7/9] Injecting authentication into config."
python3 /actor/inject_auth.py IW_TOKEN /home/hcdp_tapis_ingestor/station_values/config.json

echo "[task.sh] [8/9] Injecting configuration data into config."
python3 /actor/inject_conf.py DRIVER_CONF /home/hcdp_tapis_ingestor/station_values/config.json

echo "[task.sh] [9/9] Ingesting station values."
