...
wget https://ikeauth.its.hawaii.edu/files/v2/download/public/system/ikewai-annotated-data/HCDP/production/rainfall/new/day/statewide/partial/station_data/`date --date='last month' +%Y/%m`/rainfall_new_day_statewide_partial_station_data_`date --date='last month' +%Y_%m`.csv \
     -O Statewide_Partial_Filled_Daily_RF_mm_`date --date='last month' +%Y_%m`.csv
